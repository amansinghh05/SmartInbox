...
def received_emails():
    ...
    unread_count = conn.execute("SELECT COUNT(id) FROM emails WHERE recipient_email = ? AND opened_at IS NULL", (current_email,)).fetchone()[0]
    read_count = conn.execute("SELECT COUNT(id) FROM emails WHERE recipient_email = ? AND opened_at IS NOT NULL", (current_email,)).fetchone()[0]
    spam_count = conn.execute("SELECT COUNT(id) FROM emails WHERE recipient_email = ? AND is_spam = 1", (current_email,)).fetchone()[0]
    ...
    return render_template('received_emails.html', 
                         ...
                         user_stats=user_stats)  # Pass current_id to template
